...
def add_user(telegram_id, adult, gender):
    con = sqlite3.connect('health_bot.db')
    cur = con.cursor()
    cur.execute(f'INSERT INTO user VALUES (?,?,?)',(telegram_id,adult,gender,))
    result = cur.fetchone()
    con.close()

    return result

def update_chat_status(telegram_id, chat_status):
    con = sqlite3.connect('health_bot.db')
    cur = con.cursor()
    cur.execute(f'UPDATE user SET chat_status=? where telegram_id=?',(chat_status,telegram_id,))
    result = cur.fetchone()
    con.close()
